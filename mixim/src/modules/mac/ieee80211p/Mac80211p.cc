...
void Mac80211p::initialize(int stage) {
	...
	if (stage == 0) {
		DBG << "logs, Initialize: mac: " << myMacAddr <<  std::endl;
		...
		arturoBackoff = par("arturoBackoff").boolValue();//Arturo, Active my backoff algorithm from NED files.
...
void Mac80211p::handleUpperMsg(cMessage* msg) {
	...
	/*Replaced by Arslan HAMZA CHERIF 04022014*/
	...
	switch (event) {
		case EV_TIMER_CCA: {
			...
			if (isIdle && !guardActive) {
				...
				currentNumBackoffs++;
				//Arturo, reporting currentNumBackoffs to my apps.
				//VEHICLEp test; //Change the destination program.
				//test.entra(currentNumBackoffs, currentCW, myNicId );
				...
				if (currentNumBackoffs> macMaxCSMABackoffs) {
					...
					//ARTURO Changing a little bit the message above.
					DBG << "logs, Mac80211p, Tried " << currentNumBackoffs << " backoffs, all reported a busy "
										    << "channel. Dropping the packet." <<std::endl;
					...
					//Arturo, reporting Dropped Packets to my apps.
					VEHICLEp test; //Change the destination program.
					//test.entra(statsDroppedPackets, currentCW,myNicId );
					// test.entra(1, currentCW,myNicId ); //Envio a my applicacion el evento de un drop paquet.
					...
					//ARTURO, CHanging the message below
					//DBG << "Backoff attempt Nr. " << currentNumBackoffs << ". Starting backoff timer" << std::endl;
					DBG << "logs, Mac80211p,Backoff attempt Nr. " << currentNumBackoffs << ". Starting backoff timer" << std::endl;
...
void Mac80211p::startTimer(t_mac_timer timer) {
	...
	if (timer == TIMER_BACKOFF) {
		...
		//ARTURO, changing the type of message.
		//DBG << "Backoff value is: " << backOff << ". Current contention window is " << currentCW <<  std::endl;
		DBG << "logs, Mac80211p, Backoff value is: " << backOff << ". Current contention window is " << currentCW <<  std::endl;
...
double Mac80211p::calculateBackoff() {
	...
	/* ARTURO, I think here I can play with the back-off tries to create an adaptive back-off schema.
	 * When more retries happens I'll increase the CWmin value.
	 * NOTE: FOR THE MOMENT I'll SHUT THIS PART. I GO BACK TO APPS-LAYER.
	 */


/////////// USING BACKOFF /////////
	/*if (arturoBackoff){ //Usando my Algoritmo de Backoff
		double dynamicST= ( double(currentNumBackoffs) / double(macMaxCSMABackoffs) ) * (CWMAX_11P - currentCW) + (currentCW);
		slots= intuniform(0, int(dynamicST)); //DYNAMIC computing my APPS CW_min based on the 802.11p slot time.
		EV<< "logs, Mac80211p, currentNumBackoffs:" << currentNumBackoffs <<",macMaxCSMABackoffs,"<< macMaxCSMABackoffs << ",slots,"<< slots << ",dynamicST,"<<dynamicST <<"," <<endl;
	}
	else
	{	//usando el algorithmo de base
		slots = intuniform(0,currentCW);
	}*/

/////////// USING DROPS or MISSING ALLOCATIONS /////////
	if (arturoBackoff and currentNumBackoffs == 1 ){ //if we consider that a backoff=5 is an imminent Drop.
		double dynamicST=  ( 1.0 / 1.0 ) * (CWMAX_11P - currentCW) + currentCW;
		slots= intuniform(0, int(dynamicST)); //DYNAMIC computing my APPS CW_min based on the 802.11p slot time.
		EV<< "logs, Mac80211p-DROP5, currBackoff:" << currentNumBackoffs <<",MaxBackoff,"<< macMaxCSMABackoffs << ",slots,"<< slots << ",dynamicST,"<<dynamicST <<"," << simTime() <<endl;
	}
	else
	{
		slots = intuniform(0,currentCW);
	...
	//Arturo, Original formula
	//slots = intuniform(0,currentCW);
	currentCW = std::min(currentCW * 2,conPar[priority]->getCwMax());//here I think prepares the next CW value for the next round if happens.
	...
	//ARTURO, setting the message to another one.
	EV<< "logs, Mac80211p, In Mac80211p.cc setting the backoff timer to: " << backoffTime <<", with current CW "<< currentCW<<endl;
	return backoffTime;
}
