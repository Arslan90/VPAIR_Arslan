...
Define_Module(Mac1609_4_opp);

void Mac1609_4_opp::initialize(int stage) {
...
void Mac1609_4_opp::handleSelfMsg(cMessage* msg) {
...
void Mac1609_4_opp::handleUpperMsg(cMessage* msg) {
	...
	if ((thisMsg = dynamic_cast<WaveShortMessage*>(m->getEncapsulatedPacket())) != NULL) {
		...
		//rewrite SCH channel to actual SCH the Mac1609_4_OPP is set to
...
void Mac1609_4_opp::handleLowerMsg(cMessage* msg) {
	sendUp(msg);
}

void Mac1609_4_opp::handleLowerControl(cMessage* msg) {
...
void Mac1609_4_opp::setCsState(ChannelSelectorState state) {
	csState = state;
	assert(state == CCH || state == SCH);
}

void Mac1609_4_opp::finish() {
	//clean up queues.
...
bool Mac1609_4_opp::guardActive() const {
...
double Mac1609_4_opp::timeLeftTillGuardOver() const {
...
double Mac1609_4_opp::timeLeft() const {
...
void Mac1609_4_opp::changeServiceChannel(int cN) {
