...
#include "application/traci/VEHICLEp.h"
...
Define_Module(VEHICLEp);

const simsignalwrap_t VEHICLEp::mobilityStateChangedSignal = simsignalwrap_t(MIXIM_SIGNAL_MOBILITY_CHANGE_NAME);

void VEHICLEp::initialize(int stage) {
...
//SELF-MESSAGES
void VEHICLEp::handleSelfMsg(cMessage* msg) {
...
//RECEIVED PACKETS.
void VEHICLEp::handleLowerMsg(cMessage* msg) {
...
////////////////////// IDENTIFYING  VEHICLE MESSAGE **************
    	...
    	MYDEBUG <<"logs, from VEH,"<< mac->getSrcAddr() << ",TO VEH, "<< myApplAddr() <<","<< traci->getExternalId() <<","<< simTime() << ",rx," << junctionID << ", messageSequenceVPA, " << currentMessageSequenceVPA  << ", current MessageSequence, " << currentMessageSequence <<",currentSector," << currentSector <<","<<endl;
        //EV <<"logs, backoff,RX," << currentSector << "," << traci->getExternalId() <<","<< simTime()  <<"," <<endl;
...
WaveShortMessage*  VEHICLEp::prepareWSM(std::string name, int lengthBits, t_channel channel, int priority, int rcvId, int serial) {
...
void VEHICLEp::entra(int droppedMac80211p, int currentCW, int myNicId) {
...
void VEHICLEp::onBeacon(WaveShortMessage* wsm) {
}
void VEHICLEp::onData(WaveShortMessage* wsm) {
}
